...
## Actividad: Potenciando al Agente
...
| :--- | :--- | :--- |
| **Bot Funcional** | 30% | El flujo base funciona correctamente (Ingesta + Agente + Telegram). |
| **Integración Wikipedia** | 5% | El agente consulta Wikipedia cuando no encuentra datos locales. |
| **Mensaje de Espera** | 20% | El bot envía un "Escribiendo..." antes de la respuesta del agente. |
| **Gestión de Mensajes** | 25% | El bot elimina o actualiza el mensaje de espera al entregar la respuesta final. |
| **Bienvenida (/start)** | 20% | El bot detecta el comando inicial y saluda correctamente usando lógica de nodos. |

**Entrega**: fichero workflow *.json*